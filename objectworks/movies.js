...
var totalNumberOfFilms=films.length

// console.log("total number of films are",totalNumberOfFilms);

var belowRunTimeFilms=films.filter(f=>f.runtime<140).map(f=>f.title)

// console.log(belowRunTimeFilms.length);

var highestRunTimeFilms=films.reduce((o1,o2)=>Number(o1.runtime)>Number(o2.runtime)?o1:o2)

// console.log(highestRunTimeFilms.title);

var lowestRunTimeFilms=films.reduce((o1,o2)=>Number(o1.runtime)<Number(o2.runtime)?o1:o2)

// console.log(lowestRunTimeFilms.title);

var yearWiseFilmsCount={}

// for (f of films){
//     if (f.year in yearWiseFilmsCount){
//         yearWiseFilmsCount[f.year]+=1
//     }
//     else{
//         yearWiseFilmsCount[f.year]=1
//     }
// }
films.map(f=>f.year in yearWiseFilmsCount?yearWiseFilmsCount[f.year]+=1:yearWiseFilmsCount[f.year]=1)


// console.log(yearWiseFilmsCount);

var actionMovieNames=films.filter(f=>f.genres.includes("Action")).map(f=>f.title)

// console.log(actionMovieNames);

var actionOrDramaFilms=films.filter(f=>f.genres.includes("Action")||f.genres.includes("Drama")).map(f=>f.title)

// console.log(actionOrDramaFilms);

var mostFilmsReleasedYear={}

films.map(f=>f.year in mostFilmsReleasedYear?mostFilmsReleasedYear[f.year]+=1:mostFilmsReleasedYear[f.year]=1)

var temp=Object.entries(mostFilmsReleasedYear)

result=temp.reduce((o1,o2)=>o1[1]>o2[1]?o1:o2)
